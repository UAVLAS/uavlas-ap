...
bool Uavlas::get_abs_target(Vector3f& abs_pos_ned,Vector3f& abs_vel_ned)
{
    // return false if not healthy or no NED information provided
    if (!_flags.healthy || ((_target.status & ULS_STATUS_QR1R1_ABS_NED_OK) == 0)) {
        return false;
    }
    abs_pos_ned = _target.abs_pos_ned;
    abs_vel_ned = _target.abs_vel_ned;
    return true;
}
bool Uavlas::get_rel_target(Vector3f& rel_pos_ned,Vector3f& rel_vel_ned)
{
    // return false if not healthy or no NED information provided
